...
async def llm_rate_limit_handler(_: Request, exc: LLMRateLimitError) -> JSONResponse:
    return JSONResponse(
        status_code=429, content={"detail": "LLM rate limit exceeded, try again later."}
    )
...
async def llm_unavailable_handler(_: Request, exc: LLMUnavailableError) -> JSONResponse:
    return JSONResponse(
        status_code=503, content={"detail": "LLM provider is temporarily unavailable."}
    )
...
async def llm_error_handler(_: Request, exc: LLMError) -> JSONResponse:
    return JSONResponse(
        status_code=502, content={"detail": "LLM provider returned an unexpected error."}
    )
